...
def index():
    ...
    if session_token:   # if there is a cookie with an session token, find the user object in the database and send it to the index.html
        user = db.query(User).filter_by(session_token=session_token).first()
    else:   # if there is no email cookie, set the user object as None
        user = None
